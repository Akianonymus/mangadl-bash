...
_search_manga_gmanga() {
    ...
    for name in "${NAMES[@]}"; do
        num="$((i++))"
        # break if greater than num_of_search
        ...
        num_status="$((num_latest + TOTAL_SEARCHES))" status="${manga_info[${num_status}]//null/Unknown}" &&
            case "${status}" in
                2) status="Ongoing" ;;
                3) status="Completed" ;;
                *) status="Unknown" ;;
...
_fetch_manga_details_gmanga() {
    ...
    # logic taken from here https://github.com/tachiyomiorg/tachiyomi-extensions/blob/master/src/ar/gmanga/src/eu/kanade/tachiyomi/extension/ar/gmanga/GmangaCryptoUtils.kt
    ...
    key="$(sed -n 4p <<< "${enc_data}")" &&
        key="$(openssl sha256 <<< "${key}")" && key="${key//* /}"
    iv="$(sed -n 3p <<< "${enc_data}")" &&
        iv="$(openssl base64 -A -d <<< "${iv}" | od -A n -t x1)" && iv="${iv//$'\n'/}" && iv="${iv// /}"
