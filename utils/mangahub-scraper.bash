...
_count_images_mangahub() {
    TOTAL_IMAGES="$(: "$(for page in "${PAGES[@]}"; do
        {
            : "$(grep -Eo '_3w1ww">[0-9]/[0-9]+' "${page}/${page}"_chapter | _head 1)" &&
                total_images="${_//*\//}"
            : "$(grep -Eo "https://img.mghubcdn.com/file/imghub/${SLUG}/${page}/[0-9]+.(jpg|png)" "${page}/${page}"_chapter | _head 1)" &&
                extension="${_//*.jpg/jpg}" && extension="${extension//*.png/png}"
